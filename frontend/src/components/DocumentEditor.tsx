    ...
    useEffect(() => {
        ...
        const connectCollaboration = async () => {
            try {
                ...
                // 6. 配置 Yjs 插件（需要在编辑器初始化后）
                if (isMounted && editor && type) {
                    editor.setOptions({
                        extensions: [
                            StarterKit,
                            Placeholder.configure({
    ...
    return (
        <div className="border-2 border-blue-300 rounded-lg bg-white shadow-lg">
            ...
            <div className="px-4 py-3 border-b-2 border-gray-200">
                {!isConnected ? (
                    <div className="flex items-center justify-center text-sm text-yellow-700 bg-yellow-50 rounded-md px-4 py-2">
                        ...
                        <span>⚠️ 正在连接协作服务器...</span>
                    </div>
                ) : (
                    <div className="flex items-center justify-center text-sm text-green-700 bg-green-50 rounded-md px-4 py-2">
            ...
            {/* 编辑器内容区域 */}
            <div className="bg-white border-t-2 border-gray-200 focus-within:ring-4 focus-within:ring-blue-200 focus-within:border-blue-400 transition-all rounded-b-lg">
