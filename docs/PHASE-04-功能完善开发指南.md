...
- [x] **用户列表管理**（参考《需求文档》6.2）
  - [x] 后端：支持分页、筛选、排序的用户列表 API
  - [x] 前端：管理界面展示基础信息、搜索、批量操作
- [x] **用户权限调整**
  - [x] 后端：提供角色／权限调整接口（基于 RBAC + ACL）
  - [x] 前端：角色切换、权限矩阵、操作确认
- [x] **用户行为分析**
  - [x] 数据：记录登录日志、文档活跃度等指标
  - [x] 后端：汇总统计接口；前端：可视化图表/看板
- [x] **用户满意度调查**
  - [x] 前端：在合适入口弹出调查问卷或反馈表单
  - [x] 后端：收集与分析反馈结果
