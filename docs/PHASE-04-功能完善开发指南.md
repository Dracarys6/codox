...
- [x] **ACL 更新接口**
  ...
  - [x] 后端：防止删除 owner 权限
  - [x] 后端：使用事务确保数据一致性
  - [x] 测试：验证权限检查、数据更新和异常处理

- [x] **前端集成**
  - [x] 验证 `AclManager` 组件与后端接口的集成
  - [x] 测试 ACL 的增删改查功能

> 状态：接口与前端功能已在 2025-11-19 合入，尚缺端到端 / 自动化测试用例以及异常场景回归。

### 1. 实时通讯模块（优先级：最高 ⭐⭐⭐）

- [x] **内置聊天功能（核心已交付）**
  - [x] 数据库：创建 `chat_room` / `chat_room_member` / `chat_message` / `chat_message_read` 表
  - [x] 后端：聊天室管理接口（创建、拉人；退出逻辑待规划）
  - [x] 后端：消息发送 / 已读回执接口
  - [x] 后端：消息历史查询接口（分页 + 游标）
  - [x] 后端：WebSocket 聊天服务（collab-service `chat-handler`）
  - [x] 前端：聊天室列表组件
  - [x] 前端：聊天消息界面
  - [] 前端：实时消息收发 & 未读数量
  - [] 前端：消息状态（未读徽标 / last_read 提示）
  - [ ] 前端：@提及功能
  - [ ] 前端：文件消息上传 / 预览 / 下载

- [x] **文档内聊天**
  - [] 前端：文档编辑页面集成聊天面板
  - [] 前端：文档上下文聊天（根据 docId 自动切换房间）
  - [x] 后端：文档聊天室自动创建/复用（DocumentController + ChatController 协同）

- [x] **文件共享**
  - [x] 后端：聊天文件上传接口（`POST /api/chat/rooms/{id}/files`、20MB 限制，写入 `chat_message`)
  - [x] 后端：文件存储（复用 MinIO，SigV4 授权 `chat/room-{id}/` 目录）
  - [x] 前端：聊天文件上传组件（支持进度提示与失败重置）
  - [x] 前端：文件预览功能（图片自动拉取 Blob 预览，其它类型显示文件名）
  - [x] 前端：文件下载功能（授权请求 → Blob → 下载/另开窗口）

### 2. 通知系统增强（优先级：高 ⭐⭐）

- [x] **通知分类与过滤**
  - [x] 后端：按通知类型过滤（comment、task_assigned、task_status_changed、permission_changed等）
  - [x] 后端：按日期范围过滤
  - [x] 后端：按文档ID过滤
  - [x] 后端：通知历史记录
  - [x] 前端：通知类型筛选器
...
- [x] **版本存储**
  - [x] 数据库：创建 `doc_version` 表，记录版本号、快照、变更摘要
  - [x] 后端：保存版本、获取版本列表、单版本详情接口
  - [x] 与 MinIO/协作快照联动，支持大文件存储

- [x] **版本比较与恢复**
  - [x] 后端：提供 diff 数据（基于 Yjs 更新或文本 diff，并在缺少纯文本时从 HTML 自动提取）
  - [x] 前端：版本时间线、版本预览、差异对比视图（支持无内容时自动补拉）
  - [x] 前端：一键回滚至历史版本（恢复后会自动刷新编辑器并生成 restore 版本）

- [x] **版本策略**
  - [x] 自动版本（协作编辑器默认 60s 自动保存快照）
  - [x] 手动版本（用户点击“保存版本”）
  - [x] 版本保留策略、清理任务
...
- [x] **健康检查增强**
  - [x] 后端：`HealthController` 覆盖数据库 / Meilisearch / MinIO 探测，异常时返回 503
- [x] **MinIO 存储链路**
  - [x] 后端：`MinIOClient` 实现 SigV4 签名、上传/下载接口
  - [x] 协作快照 / 文档编辑器：全链路 Base64 → MinIO 上传、下载加载逻辑
- [ ] **Prometheus Metrics**
  - [ ] 后端：暴露 `/metrics`、添加请求数/延迟等指标
  - [ ] 基础设施：Prometheus + Grafana 仪表盘
- [ ] **集中日志 & 告警**
  - [ ] Loki / ELK 集成，结构化日志
  - [ ] 告警规则、SLO 仪表盘
- [ ] **移动端 / PWA 支持**
