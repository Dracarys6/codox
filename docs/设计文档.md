...
- **数据库**：openGauss（PostgreSQL 协议，优先 `libpqxx` 直连；本项目选型为 libpqxx）。
- **缓存/消息**：Redis（验证码、限流、会话黑名单、Pub/Sub、分布式锁）。
- **对象存储**：MinIO/S3（头像/附件/文档快照）。
- **全文检索（选做）**：Meilisearch 或 Elasticsearch。
- **部署**：Docker Compose；Nginx/Caddy 反向代理与 TLS；环境管理 `.env`。
...
- 数据与基础设施层（openGauss、Redis、对象存储、检索）

---

### 3. 数据库模型（openGauss / PostgreSQL 方言）
...
- 操作系统（本项目选型）：WSL2 + Ubuntu 20.04（推荐）；亦可使用原生 Ubuntu 20.04/22.04。
- 编译器：
  - WSL2 Ubuntu：GCC 11+ 或 Clang 15+
  - CMake 3.24+
- 运行时与工具：
...
- 中间件版本：
  - openGauss（或本地开发临时用 PostgreSQL 14+）
  - Redis 7+
  - MinIO（S3 兼容）
  - Meilisearch 1.7+（或 Elasticsearch 8+）

#### 7.2 主要依赖（C++ 服务）

- Drogon 1.8+
- libpq / libpqxx（openGauss/PostgreSQL）
- jwt-cpp（JWT）
- bcrypt（密码哈希）
...
- WSL2 Ubuntu 20.04：
  ...
  - Drogon 可用源码或包管理安装，或使用 CMake FetchContent 集成
  - 其余库可通过 `apt` 或源码安装（如 `sudo apt install -y libspdlog-dev`）
...
- 使用 `docker compose up -d` 启动：openGauss、Redis、MinIO、Meilisearch、y-websocket、C++ 服务。
- 端口规划（可调整）：
  - C++ API：8080
  - y-websocket：1234
...
COLLAB_WS_URL=ws://y-websocket:1234
COLLAB_SNAPSHOT_WEBHOOK=http://cpp-service:8080/api/collab/snapshot
...
1) 基础账号与文档 CRUD、ACL、头像上传
2) 富文本编辑（单人）+ 自动保存 + 列表/筛选
3) 引入 Yjs 实时协作 + 光标同步
4) 评论/@、通知、任务
5) 版本历史/回滚、全文检索、监控审计
