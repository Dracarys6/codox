# 多人在线协作编辑系统 - 详细设计（Ubuntu 20.04 + PostgreSQL + libpqxx）

## 1. 构建与运行

- 依赖：`build-essential cmake libpq-dev libpqxx-dev libssl-dev postgresql postgresql-contrib redis-server`
- 构建：
...
- 配置：`config.json` 指向本地 PostgreSQL；使用 `./start-local.sh` 一键启动（可切换 openGauss）。
...
```json
{
  ...
  "rdbms": [
    {
      "name": "default",
  "rdbms": "postgresql",  // 使用 PostgreSQL 协议连接 openGauss
      "host": "127.0.0.1",
      "port": 5432,
      "dbname": "collab",
      "user": "collab",
...
### 版本发布/回滚：更新 `document.last_published_version_id`，刷新索引
...
- 仅使用 Docker 运行 openGauss 容器；其余依赖本机安装。
- 健康检查：`/health` 返回 200；后续扩展为 DB/Redis 依赖探活。
