...
```typescript
// 导入 Markdown（支持文件上传或文本输入）
await apiClient.importMarkdown(file);  // 文件上传
await apiClient.importMarkdown({ markdown: "...", title: "..." });  // 文本输入
...
在主页、文档列表页面：
1. 点击"导入文档"按钮
2. 上传 Markdown 文件（.md 或 .markdown）或直接粘贴 Markdown 内容
3. （可选）输入文档标题
4. 导入成功后自动跳转到文档编辑页
...
4. **格式支持**：
   - Markdown: 支持标准 Markdown 语法，支持文件上传（.md, .markdown）和文本输入
   - Word 导出: 仅支持 .docx 格式导出
   - PDF 导出: 支持中文字符（使用 Noto Sans CJK 字体）
5. **内容处理**：
...
- **功能**: Markdown 文档导入弹窗
- **特性**:
  - Markdown 文件上传（拖拽或点击选择，支持 .md 和 .markdown）
  - Markdown 文本输入（直接粘贴内容）
  - 文件类型和大小验证（最大 50MB）
  - 加载状态和错误提示
