...
本文档描述 codox 多人在线协作文档系统（2025.11 Release）的整体架构设计，包括系统目标、技术架构、模块划分与数据存储策略。
...
- **通知系统**：站内通知、未读计数、Webhook/WS 推送、筛选能力
...
- 用户注册 / 登录 / Token 刷新、个人资料管理
- JWT 鉴权、密码重置、Refresh Token 机制
- Admin / Editor / Viewer 角色模型、管理员操作审计

### 2. 文档与权限

- 文档 CRUD、标签、状态管理、ACL（owner/editor/viewer）
- 快照持久化（MinIO）与 SHA256 校验
- 文档导入导出（Word / PDF / Markdown，doc-converter-service）

### 3. 版本管理

- 自动/手动版本策略、版本时间线与过滤器
- 单版本详情（HTML/文本）、版本 Diff API、一键恢复
- 版本审计与来源追踪（auto/manual/rollback/import 等）

### 4. 协作与通知

- 协作令牌生成、Yjs WebSocket 房间、快照引导/回调
- 通知中心（列表、筛选、未读计数）
- `/ws/notifications` 推送通道，与前端 `VITE_NOTIFICATION_WS_URL` 对接

### 5. 评论 / 任务 / 运营

- 树形评论、@提及、锚点定位
- 任务分配、状态流转、看板视图
- 管理员用户管理（列表筛选、CSV 导出、状态/角色调整）与活跃度/满意度分析

### 6. 搜索与审计

- Meilisearch 索引同步、关键词搜索与权限过滤
- 管理员操作日志、健康检查、指标上报（可选接入 Prometheus / Grafana）
...
| `password_reset_token` | 密码重置令牌存储 |
| `document` | 文档元数据（状态、锁定、版本策略） |
| `doc_acl` | 文档访问控制列表 |
| `document_version` | 文档版本快照元数据（Diff/恢复信息） |
| `tag` | 标签 |
| `doc_tag` | 文档标签关联 |
| `comment` | 评论（树形结构） |
| `task` | 任务 |
