...
app.post('/convert/text-to-pdf', async (req, res) => {
    try {
        ...
        if (hasNonAscii) {
            ...
            // 2）再尝试常见系统字体路径（Linux / Windows / macOS）
            ...
            for (const fontPath of candidatePaths) {
                ...
                // pdf-lib 不支持 TTC，直接跳过，避免 this.font.layout is not a function 报错
                const ext = path.extname(fontPath).toLowerCase();
                if (ext === '.ttc') {
                    console.warn('Skip TTC font (unsupported by pdf-lib):', fontPath);
                    continue;
                }
                ...
                try {
                    ...
                    // 注意：部分自定义字体在 subset 模式下会触发 _this.subset.encodeStream 报错，
                    // 这里使用默认模式以保证兼容性。
            ...
            if (!fontLoaded) {
                console.warn(
                        'No Chinese font found, fallback to Helvetica. Non-ASCII characters may not render correctly.');
        ...
        try {
            lines.forEach(line => {
                ...
                try {
                    ...
                    // 尝试绘制一个占位符
                    try {
                        currentPage.drawText('[Text rendering error]', {x: margin, y: y, size: fontSize, font: font});
