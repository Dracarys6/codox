...
你的 `JwtUtil.cc` 中有几个问题：

1. 函数声明重复了（`.h` 已经声明，`.cc` 不需要再声明）
2. `.cc` 文件中缺少 `#include` 必要的头文件
3. 函数没有实现体

### 实现方案选择

**方案1：使用 jwt-cpp 库（推荐，简单）

需要先在 CMakeLists.txt 中添加：
...
如果不想引入外部库，可以用 OpenSSL 手动实现 JWT。

---

### JwtUtil.cc 实现框架（使用 jwt-cpp）
...
std::string JwtUtil::generateToken(int userId, const std::string& secret, int expiresIn) {
    // 1. 计算过期时间戳（当前时间 + expiresIn 秒）
    auto now = std::chrono::system_clock::now();
    auto exp = now + std::chrono::seconds(expiresIn);
    
    // 2. 创建 payload（包含 user_id 和 exp）
...
### JwtUtil.cc 实现框架（手动实现 - OpenSSL）

如果你选择手动实现，需要：

1. Base64URL 编码/解码
2. HMAC-SHA256 签名
3. JSON 创建和解析（可以用 jsoncpp）

这会更复杂，建议先用 jwt-cpp 库，理解原理后再考虑手动实现。
...
**方案1：使用 libbcrypt（如果有的话）**

```cpp
#include "PasswordUtils.h"
#include <bcrypt/bcrypt.h>  // 需要安装 libbcrypt-dev
...
**方案2：使用 OpenSSL + SHA-256 + 随机盐（推荐，不需要额外库）**
...
```cmake
# 在 CMakeLists.txt 的 set(ALL_SOURCES ...) 部分添加：
set(ALL_SOURCES
    src/main.cpp
...
- 如果使用 jwt-cpp，确保 CMakeLists.txt 中正确添加了 FetchContent
- 或者手动安装：`sudo apt install libjwt-dev`（如果可用）

### 2. 编译错误："找不到 openssl/sha.h"
...
然后在 CMakeLists.txt 中已经链接了 `ssl` 和 `crypto`，应该没问题。
...
- secret 必须一致
- 时间戳计算是否正确（注意时区）
- Base64URL 编码是否正确（如果有手动实现）

### 4. 密码验证总是失败

**检查点：**

- 哈希算法是否一致（SHA-256 vs BCrypt）
- 盐的存储和提取是否正确
- 字符串编码问题（UTF-8）
...
- **jwt-cpp 文档**：<https://github.com/Thalhammer/jwt-cpp>
- **OpenSSL SHA-256**：`man SHA256` 或查看 OpenSSL 文档
- **Drogon 数据库**：<https://drogon.docsforge.com/master/pages/09-7-Database/>

---

## ✅ 实现检查清单

- [ ] JwtUtil.cc 实现完整（生成/验证/解析）
- [ ] PasswordUtils.cc 实现完整（哈希/验证）
- [ ] DbUtils.cc 实现基本功能
- [ ] CMakeLists.txt 已添加工具类文件
- [ ] 编译通过
- [ ] 简单测试验证功能正常

**开始实现吧！遇到具体编译错误或运行时问题，随时问我！** 🚀
